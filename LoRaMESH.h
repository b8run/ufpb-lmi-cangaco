...
#define LoRa_LOGICAL_LEVEL_HIGH HIGH
#define LoRa_LOGICAL_LEVEL_LOW  LOW
...
class LoRaMESH{
    ...
    public:
        ...
        uint8_t digitalLoraRead(int16_t id, uint8_t gpio){
            getGPIOStatus(id, gpio);
        ...
        bool digitalLoraWrite(int16_t id, uint8_t gpio, uint8_t logical_level){
            if(gpio < 0x00 || gpio > 0x07)
                return false;
            else if(logical_level < 0x00 || logical_level > 0x03)
                return false;
